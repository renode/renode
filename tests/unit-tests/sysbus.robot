...
Test Registering Mapped Memory In Locked Range
    ...
    Provides                   sysbus-with-mock-cpus-in-locked-and-unlocked-mem

Test Instruction Fetch In Locked Range
    ...
    [Timeout]                  20 seconds
    Requires                   sysbus-with-mock-cpus-in-locked-and-unlocked-mem
    Set Crash On Abort         False

    # Now, to really test if newly registered memory has been locked correctly, try executing code (instructions don't matter here). CPU0 (in locked memory) should abort due to the locked memory
    Execute Command            mockCpu1 IsHalted true
...
Test Instruction Fetch In Unlocked Range
    [Timeout]                  20 seconds
    Requires                   sysbus-with-mock-cpus-in-locked-and-unlocked-mem
    Set Crash On Abort         False

    # CPU1 (in unlocked memory) should abort due to going into an unmapped address
    Execute Command            mockCpu0 IsHalted true
    ${log}=  Wait For Log Entry    CPU abort  timeout=1
