...
sed_inplace 's#tools/#tests/#' $TEST_SCRIPT
sed_inplace 's#--properties-file.*#--robot-framework-remote-server-full-directory=${ROOT_PATH}/bin --css-file=${ROOT_PATH}/tests/robot.css -r $(pwd) "$@"#' $TEST_SCRIPT
...
sed_inplace "s|/opt/renode|$DIR|" $COMMAND_SCRIPT
