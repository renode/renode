...
namespace Antmicro.Renode.Peripherals.CoSimulated
{
    public class CoSimulatedPeripheral : ICoSimulationConnectible, IQuadWordPeripheral, IDoubleWordPeripheral, IWordPeripheral, IBytePeripheral, IBusPeripheral, IDisposable, INumberedGPIOOutput, IGPIOReceiver, IAbsoluteAddressAware
    {
            ...
            ulong renodeToCosimSignalsOffset = 0, Range cosimToRenodeSignalRange = default(Range))
        {
            ...
            if(this.cosimToRenodeSignalRange.Size != 0)
            {
                for(int i = 0; i < (int)this.cosimToRenodeSignalRange.Size; i++)
                {
        ...
        public virtual void OnConnectionAttached(CoSimulationConnection connection)
        {
            ...
            {
                this.connection.RegisterOnGPIOReceive(ReceiveGPIOChange, cosimToRenodeSignalRange);
            }
        }
        ...
        public virtual void OnConnectionDetached(CoSimulationConnection connection)
        {
            if(cosimToRenodeSignalRange.Size != 0)
            {
                this.connection.UnregisterOnGPIOReceive(cosimToRenodeSignalRange);
            }
        ...
        public virtual void ReceiveGPIOChange(int coSimNumber, bool value)
        {
            if(cosimToRenodeSignalRange.Size == 0)
            {
                this.Log(LogLevel.Warning, $"Received GPIO change from co-simulation, but no cosimToRenodeSignalRange is defined. Consider defining cosimToRenodeSignalsOffset and numberOfCosimToRenodeSignals in the platform definition.");
                return;
            }
